...
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            return 'Please add ${0} more funds.'.format(self.price)
        elif self.stock != 0 and self.balance < self.price:
            balance_less = self.price - self.balance
            return 'Please add ${0} more funds.'.format(balance_less)
        elif self.stock != 0 and self.balance > self.price:
            self.stock -= 1
...
class Game:
    ...
    def play(self):
        while not self.game_over():
            if self.turn == 0 or self.turn == 2 or self.turn == 4 or self.turn == 6 or self.turn == 8:
                self.p1.choose(self.p2)(self.p2)
            elif self.turn == 1 or self.turn == 3 or self.turn == 5 or self.turn == 7 or self.turn == 9:
